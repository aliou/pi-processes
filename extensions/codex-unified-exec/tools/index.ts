 ...
 *
 * Schemas + descriptions are ported from codex's tool specs
 ...
 * pipe-only; tty:true is rejected until PTY support lands.
 */
...
export const ExecCommandParams = Type.Object({
  ...
  tty: Type.Optional(
    Type.Boolean({
      description:
        "Reserved for future PTY support; true is rejected in this build. Omit or set false for pipe stdio.",
    }),
  ),
  ...
  max_output_tokens: Type.Optional(
    Type.Number({
      description:
        "Output token budget. Defaults to 10000 tokens; clamped to the session ceiling.",
    }),
  ),
  shell: Type.Optional(
    Type.String({
...
export function registerCodexExecTools(
  ...
  pi.registerTool(
    defineTool({
      ...
      label: "Exec command",
      description:
        "Runs a command in a subprocess with pipe stdio, returning output or a session ID for ongoing interaction.",
      promptSnippet:
