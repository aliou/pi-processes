 ...
 *     open so interactive pipe sessions work without a PTY. An explicit
 *     `tty: true` on exec_command is rejected (see execCommand) until PTY
 *     support lands, rather than silently advertising a PTY it does not provide.
 *   - `\u0003` (Ctrl-C / ETX) is sent as SIGINT to the process group, mirroring
 *     codex's interrupt() (non-tty) and PTY ^C (tty).
 *   - Released/evicted sessions also reap terminal records + log files from the
 *     isolated ProcessManager (clearFinished), so completed codex commands do
 *     not accumulate hidden disk usage. This is host-repo hygiene, not a codex
 *     semantic; codex sessions are hidden from /ps and therefore unclearable by
 *     the user.
 */
...
export interface ProcessManagerLike {
  ...
  killAll(): void;
  /** Reap terminal process records and their log files. */
...
export class SessionManager {
  ...
  async execCommand(req: ExecCommandRequest): Promise<ExecCommandOutput> {
    ...
    // Pipe-only: PTY allocation is not yet supported. Reject an explicit tty
    // request rather than silently falling back to pipes. codex advertises a PTY
    // for tty:true, which would mislead commands that branch on TTY presence or
    // prompt interactively.
    if (req.tty === true) {
      throw new Error(
        "exec_command does not support tty:true in this build (pipe-only)",
      );
    }
   ...
   * process group (LRU eviction). A released codex id is freed for reuse, and
   * terminal records + their log files are reaped from the isolated manager so
   * finished codex commands do not leak disk (these sessions are hidden from
   * /ps and therefore unclearable by the user).
   */
