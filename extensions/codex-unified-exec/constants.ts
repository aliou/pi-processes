 ...
 * an explicit value wins, otherwise the default. The result is clamped to
 * UNIFIED_EXEC_OUTPUT_MAX_TOKENS so a very large request cannot return the full
 * raw buffer (codex's truncation_policy.token_budget() cap).
 */
export function resolveMaxTokens(maxTokens: number | undefined): number {
  const value = maxTokens ?? DEFAULT_MAX_OUTPUT_TOKENS;
  return Math.min(value, UNIFIED_EXEC_OUTPUT_MAX_TOKENS);
}
